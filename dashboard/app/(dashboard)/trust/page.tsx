    ...
    <section className="space-y-4">
      ...
      <Card title="Scores">
        {loading ? (
          ...
          <p className="text-sm text-bpvp-muted">{isSpanish ? "Aun no hay puntajes." : "No scores yet."}</p>
        ) : (
          <ul className="space-y-1 text-sm">
            {scoreRows.map(([k, v]) => (
              <li key={k}>
                <span className="font-mono text-xs text-bpvp-ink">{k}</span>
                <span className="text-bpvp-faint"> — </span>
                {v}
                <span className="text-bpvp-faint"> ({data?.ratings?.[k] ?? "—"})</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <Card title="Record score update">
        <form className="grid gap-3 text-sm md:grid-cols-4 md:items-end" onSubmit={onScore}>
          <label className="block space-y-1 md:col-span-2">
            <span className="text-bpvp-muted">{isSpanish ? "Sujeto (desk / id entidad)" : "Subject (desk / entity id)"}</span>
            <input name="subject" required placeholder={isSpanish ? "ej. desk-latam-01" : "e.g. desk-latam-01"} className="w-full bpvp-field font-mono text-xs" />
          </label>
          <label className="block space-y-1">
            <span className="text-bpvp-muted">{isSpanish ? "Puntaje (0-100)" : "Score (0–100)"}</span>
            <input name="score" type="number" min={0} max={100} step="0.1" required placeholder={isSpanish ? "ej. 82.5" : "e.g. 82.5"} className="w-full bpvp-field" />
          </label>
          <label className="block space-y-1">
            <span className="text-bpvp-muted">{isSpanish ? "Etiqueta de rating" : "Rating label"}</span>
            <input name="rating" required placeholder={isSpanish ? "ej. GREEN" : "e.g. GREEN"} className="w-full bpvp-field" />
          </label>
