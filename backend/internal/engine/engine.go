...
type Module interface { Name() string; ApplyEvent(models.DomainEvent) error; ApplyAction(models.Action) error; GetState() any }
...
func (r *Registry)Module(name string)(Module,bool){m,ok:=r.modules[name]; return m,ok}
func (r *Registry)ActiveModules()[]string{out:=make([]string,0,len(r.modules)); for k:=range r.modules{out=append(out,k)}; sort.Strings(out); return out}
...
func (e *Engine)ApplyAction(a models.Action) error {
    if err:=validation.ValidateAction(a); err!=nil{return err}
    e.mu.Lock()
    defer e.mu.Unlock()
    m,ok:=e.registry.Module(a.Module)
    if !ok{return fmt.Errorf("%w: %s",errors.ErrModuleNotFound,a.Module)}
    if err := m.ApplyAction(a); err != nil { return err }
    if e.journal != nil {
...
func (e *Engine)ApplyEvent(evt models.DomainEvent) error {
    ...
    if err:=validation.ValidateEvent(evt); err!=nil{return err}
    e.mu.Lock()
    defer e.mu.Unlock()
    m,ok:=e.registry.Module(evt.Module)
    if !ok{return fmt.Errorf("%w: %s",errors.ErrModuleNotFound,evt.Module)}
    if evt.Height>e.height{e.height=evt.Height}
    if err := m.ApplyEvent(evt); err != nil { return err }
    if e.journal != nil {
...
func (e *Engine)Status() map[string]any { return map[string]any{"service":"axe-engine","version":e.version,"height":e.height,"stateHash":e.stateHash,"modules":e.registry.ActiveModules()} }
func (e *Engine)ModuleState(module string)(json.RawMessage,error){m,ok:=e.registry.Module(module); if !ok{return nil,fmt.Errorf("%w: %s",errors.ErrModuleNotFound,module)}; raw,err:=json.Marshal(m.GetState()); if err!=nil{return nil,err}; return raw,nil}
