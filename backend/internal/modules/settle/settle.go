...
type State struct {
	...
	Records      []Settlement `json:"records"`
}
type Module struct {
	mu    sync.RWMutex
	state State
...
func New() *Module {
	return &Module{state: State{Liquidations: []string{}, Payments: []string{}, Records: []Settlement{}}}
}
func (m *Module) Name() string { return "settle" }
func (m *Module) ApplyEvent(e models.DomainEvent) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.state.Payments = append(m.state.Payments, e.TxID)
	return nil
}
func (m *Module) ApplyAction(a models.Action) error {
	...
	m.state.Records = append(m.state.Records, Settlement{ID: id, Status: "settled"})
	return nil
}
