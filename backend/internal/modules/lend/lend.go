...
func New() *Module {
	return &Module{state: State{Pools: map[string]float64{"main": 1000}, APY: map[string]float64{"main": 4.5}, Positions: []Position{}}}
}
...
func (m *Module) ApplyAction(a models.Action) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	u, _ := a.Data["user"].(string)
	c, _ := a.Data["collateral"].(float64)
	d, _ := a.Data["debt"].(float64)
	m.state.Positions = append(m.state.Positions, Position{User: u, Collateral: c, Debt: d, APY: m.state.APY["main"]})
	return nil
}
